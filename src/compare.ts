...
function getThresholdsForScenario(
  ...
  return {
    maxIncreasePercent:
      override?.maxIncreasePercent ?? thresholds.defaultMaxIncreasePercent,
    maxErrorRateIncreasePercent:
...
function compareScenario(
  ...
  // Compare error rate
  const errorRateDelta = computeDeltaPercent(
    baseline.errorRate,
    current.errorRate,
  );
  const errorRateStatus = evaluateStatus(
    errorRateDelta,
    maxErrorRateIncreasePercent,
  );
  deltas.push({
  ...
  const reqDelta = computeDeltaPercent(
