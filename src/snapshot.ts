...
export function histogramQuantile(
    ...
    for (let i = 0; i < buckets.length; i++) {
        if (buckets[i].count >= rank) {
            ...
            return (
                bucketLower +
                ((bucketUpper - bucketLower) * (rank - countLower)) / (countUpper - countLower)
            )
        }
    }
...
function computePercentiles(buckets: Array<{ le: number; count: number }>): PercentileValues {
    return {
...
function discoverScenarioIterations(
    ...
    const result = new Map<ScenarioIterationKey, { scenario: string; iteration: number }>()
    ...
    for (const name of relevantMetrics) {
        ...
        for (const v of metric.values) {
            ...
            if (scenario !== undefined && iteration !== undefined) {
                ...
                if (!result.has(key)) {
                    result.set(key, { scenario: String(scenario), iteration: Number(iteration) })
                }
            }
        }
    }
...
function buildScenarioMetrics(metricsMap: Map<string, MetricData>): ScenarioMetrics[] {
...
function buildResourceMetrics(metricsMap: Map<string, MetricData>): ResourceMetrics[] {
    ...
    const zeroPercentiles: PercentileValues = { p50: 0, p75: 0, p90: 0, p95: 0, p99: 0 }
